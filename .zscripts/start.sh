...
# Use persistent database path - this prevents data loss on redeployment
PERSISTENT_DB_PATH="/home/z/my-project/db/custom.db"
PERSISTENT_DATABASE_URL="file:$PERSISTENT_DB_PATH"
...
if [ -f "./next-service-dist/server.js" ]; then
    ...
    export DATABASE_URL="${DATABASE_URL:-$PERSISTENT_DATABASE_URL}"

    if [ "$DATABASE_URL" = "$PERSISTENT_DATABASE_URL" ]; then
        if [ ! -f "$PERSISTENT_DB_PATH" ]; then
            echo "⚠️  未找到数据库文件 $PERSISTENT_DB_PATH，将自动创建"
        fi
        echo "🗄️  当前使用持久化数据库: $PERSISTENT_DB_PATH"
    else
