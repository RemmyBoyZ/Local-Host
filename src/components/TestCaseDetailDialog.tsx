...
import {
  ...
  Filter, Keyboard, Layers, Loader2, Maximize2, Minimize2, MousePointerClick, Pause, Pencil, Play, Plus, RefreshCw, Search, Sparkles, Square, Trash2, Wrench, X, Minus, Volume2, VolumeX
} from 'lucide-react';
    ...
    return (
      <div className="fixed inset-0 z-[100] flex flex-col bg-black">
        <div className="flex h-14 shrink-0 items-center justify-between border-b border-white/10 bg-black/80 px-5 backdrop-blur">
          ...
          {targetUrl && (
            <p className="hidden truncate text-[11px] text-slate-500 sm:block max-w-lg">{targetUrl}</p>
          )}
          <Button
            type="button"
            variant="ghost"
        ...
        >
          <div className="mb-3 flex items-center gap-3">
            ...
            <span className="min-w-[36px] text-[10px] font-bold text-slate-400">
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              ...
              {targetUrl && (
                <span className="ml-auto truncate text-[9px] text-white/40 max-w-[200px]">{targetUrl}</span>
              )}
            </div>
          </div>
        </div>
    ...
    detailStepLogs
      ...
      .map((log, index) => {
        ...
        return {
          ...log,
          detailStepKey,
          detailStep: { ...log.detailStep, ...(detailStepEdits[detailStepKey] || {}) },
        };
      })
                        ...
                        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2">
                              ...
                              <Badge
                                variant="outline"
                                className={`rounded-md text-[10px] font-bold shadow-none ${socketReady ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-amber-200 bg-amber-50 text-amber-700'}`}
                              >
                      ...
                      <div className="flex items-center justify-between mb-4 shrink-0">
                        ...
                        <div className="flex items-center gap-2 bg-slate-100 p-1 rounded-lg">
                          ...
                          <Button
                            ...
                            disabled={isLoadingHistory}
                          >
                            {isLoadingHistory ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <History className="w-3.5 h-3.5" />}
                                    ...
                                    >
                                      <div className="grid w-full grid-cols-12 items-start gap-3 p-3 text-left">
                                        ...
                                        >
                                          {isEditing ? (
                                            <div className="space-y-2">
                                              <Input
                                                value={step.label || ''}
                                                onChange={(e) => updateDetailStep(logId, { label: e.target.value })}
                                              ...
                                              <Input
                                                value={step.value || ''}
                                                onChange={(e) => updateDetailStep(logId, { value: e.target.value })}
                                                placeholder="Value"
                                            ...
                                            <>
                                              <span className="block truncate font-sans text-xs font-bold text-slate-200">{getDetailStepTitle(log)}</span>
                                              {step?.value && <span className="mt-1 block truncate font-sans text-[11px] text-emerald-300">Value: {step.value}</span>}
                                        ...
                                        <span className="col-span-2 flex justify-end gap-1">
                                          <Button type="button" variant="ghost" size="sm" className="h-7 w-7 p-0 text-slate-500 hover:bg-emerald-950/40 hover:text-emerald-300" onClick={() => setEditingDetailStepKey(isEditing ? null : logId)}>
                                            {isEditing ? <Check className="h-3.5 w-3.5" /> : <Pencil className="h-3.5 w-3.5" />}
                                          </Button>
                                          <Button type="button" variant="ghost" size="sm" className="h-7 w-7 p-0 text-slate-500 hover:bg-rose-950/40 hover:text-rose-300" onClick={() => deleteDetailStep(logId)}>
                                            <Trash2 className="h-3.5 w-3.5" />
                                          </Button>
                                          <Button type="button" variant="ghost" size="sm" className="h-7 w-7 p-0 text-slate-500 hover:bg-slate-900 hover:text-slate-200" onClick={() => setExpandedLogId(expandedLogId === logId ? null : logId)}>
                                ...
                                consoleLogs.map((log, index) => {
                                  ...
                                  return (
                                    <div key={logId} className={`group ${isError ? 'bg-rose-950/20' : isWarning ? 'bg-amber-950/20' : 'hover:bg-white/5'}`}>
                                      <div className="flex items-start gap-3 p-2 cursor-pointer transition-colors" onClick={() => setExpandedLogId(expandedLogId === logId ? null : logId)}>
                                      ...
                                      {expandedLogId === logId && typeof log.log === 'object' && (
                                        <div className="px-10 pb-3">
                                          <div className="bg-slate-900 rounded-lg p-3 border border-slate-800 shadow-inner">
                                            <pre className="text-emerald-500/80 whitespace-pre-wrap overflow-x-auto">{JSON.stringify(log.log, null, 2)}</pre>
                                          </div>
                                        </div>
                                      )}
                                    </div>
                            ...
                            <div className="h-full flex flex-col">
                              <div className="space-y-2 border-b border-slate-800 bg-slate-950 p-3">
                                <div className="flex flex-wrap items-center gap-2">
                                  ...
                                  <select value={networkFilters.host} onChange={(e) => updateNetworkFilters({ host: e.target.value })} className="h-8 min-w-[150px] rounded-md border border-slate-800 bg-slate-900 px-2 text-[11px] font-semibold text-slate-300 outline-none focus:border-indigo-500">
                                    <option value="all">All hosts ({networkLogItems.length})</option>
                                    {networkHosts.map((host) => <option key={host} value={host}>{host}</option>)}
                                  </select>
                                  <select value={networkFilters.method} onChange={(e) => updateNetworkFilters({ method: e.target.value })} className="h-8 rounded-md border border-slate-800 bg-slate-900 px-2 text-[11px] font-semibold text-slate-300 outline-none focus:border-indigo-500">
                                    <option value="all">All methods</option>
                                    {networkMethods.map((method) => <option key={method} value={method}>{method}</option>)}
                                  </select>
                                  <select value={networkFilters.status} onChange={(e) => updateNetworkFilters({ status: e.target.value })} className="h-8 rounded-md border border-slate-800 bg-slate-900 px-2 text-[11px] font-semibold text-slate-300 outline-none focus:border-indigo-500">
                                  ...
                                  <Button type="button" variant="ghost" size="sm" className="h-8 gap-1.5 px-2 text-[10px] font-bold text-slate-400 hover:bg-slate-900 hover:text-white" onClick={() => setNetworkFilters(DEFAULT_NETWORK_FILTERS)}>
                                ...
                                <div className="flex flex-wrap items-center gap-2 text-[10px]">
                                  {([
                                    ...
                                    <Button key={filter.key} type="button" variant="ghost" size="sm"
                                      className={`h-7 rounded-full border px-3 text-[10px] font-bold ${networkFilters[filter.key] ? 'border-indigo-500/40 bg-indigo-950/60 text-indigo-200 hover:bg-indigo-900/60' : 'border-slate-800 bg-slate-900 text-slate-500 hover:bg-slate-800 hover:text-slate-200'}`}
                                      onClick={() => toggleNetworkFilter(filter.key)}
                                    >
                                      {filter.label}
                                    </Button>
                                  ))}
                                  <span className="ml-auto rounded-full border border-cyan-500/20 bg-cyan-950/40 px-2 py-1 font-bold text-cyan-300">API {networkCategoryCounts.business}</span>
                                  {hiddenNetworkCount > 0 && (
                                    <span className="rounded-full border border-slate-800 bg-slate-900 px-2 py-1 font-bold text-slate-500">{hiddenNetworkCount} hidden</span>
                                  )}
                                </div>
                              </div>
                                  ...
                                  networkLogs.map(({ log: net, meta }, index) => {
                                    ...
                                    return (
                                      <div key={logId} className="group hover:bg-white/5">
                                        <div className="grid grid-cols-12 gap-2 p-2 cursor-pointer items-center transition-colors" onClick={() => setExpandedLogId(expandedLogId === logId ? null : logId)}>
                                          <div className="col-span-1 font-black text-indigo-400 truncate">{getNetworkMethod(net.network)}</div>
                                          <div className="col-span-1 truncate">
                                            <span className={`rounded border px-1.5 py-0.5 text-[9px] font-bold uppercase ${getNetworkCategoryClass(meta.category)}`}>{meta.label}</span>
                                          </div>
                                          ...
                                          <div className="col-span-2 text-center">
                                            <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${getNetworkStatusClass(net.network)}`}>{getNetworkStatus(net.network)}</span>
                                          </div>
                                          <div className="col-span-2 text-right text-slate-500">
                                            <span>{getNetworkDuration(net.network)}</span>
                                            {typeof net.relativeMs === 'number' && <span className="ml-2 text-indigo-300">{formatRelativeTime(net.relativeMs)}</span>}
                                          </div>
                                        ...
                                        {expandedLogId === logId && (
                                          <div className="p-4 bg-slate-900/50 border-t border-slate-800">
                                            <div className="mb-3 rounded-lg border border-slate-800 bg-slate-950/70 p-3">
                                              ...
                                              <pre className="whitespace-pre-wrap break-all text-[10px] leading-relaxed text-slate-300">{formatNetworkUrlForDisplay(net.network.url)}</pre>
                                            </div>
                                            <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
                                              <div className="space-y-3">
                                                <p className="text-[10px] font-bold text-slate-500 uppercase">Headers</p>
                                                <pre className="max-h-[320px] overflow-auto whitespace-pre-wrap break-words rounded border border-slate-800 bg-slate-900 p-3 text-[10px] leading-relaxed text-slate-400">{formatPrettyValue(net.network.headers)}</pre>
                                              </div>
                                              <div className="space-y-3">
                                                <p className="text-[10px] font-bold text-slate-500 uppercase">Request Payload</p>
                                                <pre className="max-h-[420px] overflow-auto whitespace-pre-wrap break-words rounded border border-slate-800 bg-slate-900 p-3 text-[10px] leading-relaxed text-cyan-300/90">{formatPrettyValue(getRequestPayload(net.network.data))}</pre>
                                              </div>
                                              <div className="space-y-3">
                                                <p className="text-[10px] font-bold text-slate-500 uppercase">Response</p>
                                                <pre className="max-h-[420px] overflow-auto whitespace-pre-wrap break-words rounded border border-slate-800 bg-slate-900 p-3 text-[10px] leading-relaxed text-emerald-500/80">{formatPrettyValue(getResponsePayload(net.network.data))}</pre>
                                              </div>
                                            </div>
                                          </div>
                                        )}
